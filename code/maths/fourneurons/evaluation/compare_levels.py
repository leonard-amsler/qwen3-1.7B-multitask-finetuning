"""Compare scored evaluation results by problem level.
...
Example:
    python -m fourneurons.evaluation.compare_levels \
        ...
        --split val \
        --output_dir /scratch/results/math/competitionmath/compare_levels
"""
...
DEFAULT_LEVEL_FIELDS = ("level", "difficulty", "math500_level")
...
def detect_level_field(rows: Iterable[dict], requested: Optional[str]) -> str:
    if requested is not None:
        return requested

    seen_rows = list(rows)
    for field in DEFAULT_LEVEL_FIELDS:
        if any(row.get(field) not in (None, "") for row in seen_rows):
            return field

    fields = ", ".join(DEFAULT_LEVEL_FIELDS)
    raise ValueError(
        f"Could not find a level field in generations rows. "
        f"Tried: {fields}. Pass --level_field explicitly."
    )
...
def sort_level_key(level: str) -> tuple[int, str]:
    ...
    if match is not None:
        return (0, f"{int(match.group(0)):010d}")
    return (1, level.lower())


def level_rows_for_result_dir(
    result_dir: Path,
    split: str,
    ...
    level_field = detect_level_field(gens_rows, level_field)
    run_label = label or result_dir.name
    by_level: dict[str, list[tuple[dict, dict]]] = defaultdict(list)

    for gen_row, detail in zip(gens_rows, details):
        level = gen_row.get(level_field)
        if level in (None, ""):
            level = "unleveled"
        by_level[str(level)].append((gen_row, detail))

    rows = []
    for level, pairs in sorted(by_level.items(), key=lambda item: sort_level_key(item[0])):
        ...
        if len(n_values) != 1:
            raise ValueError(
                f"Level {level!r} in {scored_path} has mixed completion counts: "
                f"{sorted(n_values)}"
            )
...
def plot_level(
    ...
    ax.set_title(f"Difficulty Level {level}")
...
def plot_all_levels(
    ...
    paths = []
    for level, level_rows in sorted(by_level.items(), key=lambda item: sort_level_key(item[0])):
        path = output_dir / f"{safe_filename(level)}.png"
        plot_level(level_rows, level, path, ymin=ymin, ymax=ymax)
        paths.append(path)
    return paths


def plot_level_metric_bars(
    rows: list[dict],
    output_path: Path,
    ymin: float,
    ymax: float,
    ...
    levels = sorted({row["level"] for row in rows}, key=sort_level_key)
    lookup = {(row["level"], row["run_index"]): row for row in rows}

    group_width = 0.82
    bar_width = group_width / max(len(runs), 1) * 0.92
    centers = list(range(len(levels)))
    ...
    for run_pos, (run_index, run_label) in enumerate(runs):
        ...
        values = []
        for level in levels:
            value = lookup.get((level, run_index), {}).get("pass@8")
    ...
    ax.set_title("pass@8 by Difficulty Level")
    ax.set_ylabel("pass@8 (%)")
    ax.set_xticks(centers)
    ax.set_xticklabels([f"L{level}" for level in levels])
    ax.grid(axis="y", alpha=0.3)
    ax.legend(ncol=min(3, max(len(runs), 1)), fontsize=8)
    ax.set_ylim(*percent_axis_bounds([lookup.get((level, run_index), {}).get("pass@8") for level in levels for run_index, _ in runs], ymin))
    ax.figure.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare existing scored result folders by problem level. Each result "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory where level_metrics.csv and level plots are written. "
            "Defaults to compare_levels under the common parent."
        ),
    )
    parser.add_argument(
        "--level_field",
        default=None,
        help="Generation-row field to group by. Defaults to auto-detecting level, difficulty, or math500_level.",
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def default_output_dir(result_dirs: list[Path]) -> Path:
    ...
    if len(parents) == 1:
        return next(iter(parents)) / "compare_levels"
    return Path("compare_levels")
...
def main() -> None:
    ...
    csv_path = output_dir / "level_metrics.csv"
    write_csv(rows, csv_path)
    plot_paths = plot_all_levels(rows, output_dir, ymin=args.ymin, ymax=args.ymax)
    summary_plot_path = output_dir / "all_levels_metrics.png"
    plot_level_metric_bars(rows, summary_plot_path, ymin=args.ymin, ymax=args.ymax)

    print(f"Read {len(result_dirs)} result directories.")
    print(f"Wrote level metrics CSV: {csv_path}")
    print(f"Wrote {len(plot_paths)} level plots to: {output_dir}")
    print(f"Wrote level summary plot: {summary_plot_path}")
