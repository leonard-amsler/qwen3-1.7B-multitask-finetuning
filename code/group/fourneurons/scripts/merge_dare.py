"""
DARE-TIES (Drop And REscale + TIES sign consensus) — Yu et al. 2023 + Yadav et al. 2024
...
def main(density, adapters=None, output_path=None):
    if adapters is None:
        adapters = DEFAULT_ADAPTERS
    
    if output_path is None:
        output_path = f"/scratch/checkpoints/group_model/dare_merged_density{density}"
    ...
    print(f"Merging with DARE-TIES (density={density})...")
    model.add_weighted_adapter(
        adapters = list(adapters.keys()),
        weights = weights,
        adapter_name = "dare_merged",
        combination_type = "dare_ties",  # DARE preprocessing + TIES sign consensus
        density = density,
    )
    model.set_adapter("dare_merged")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merge LoRA adapters using DARE-TIES method")
    parser.add_argument(
        "density",
        type=float,
        help="Density parameter (fraction of weights to keep, 0.0-1.0)"
    )
    parser.add_argument(
        "--adapters",
        nargs=4,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output path for merged model (default: /scratch/checkpoints/group_model/dare_merged_density{density})"
    )
    ...
    main(density=args.density, adapters=adapters, output_path=args.output)