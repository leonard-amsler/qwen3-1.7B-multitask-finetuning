...
def main(
    ...
    elif checkpoint_dir is not None and no_lora:
    ...
    sampling_params = SamplingParams(temperature=0.7, top_p=0.9, max_tokens=16384, seed=42, n=n)
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_override or model_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to a tokenizer directory to use instead of the model_dir tokenizer. This can be used if the LoRA checkpoint has a different tokenizer than the base model.",
    )
    parser.add_argument(
        "--n",
        type=int,
    ...
    parser.add_argument(
        "--no_lora",
        action="store_true",
        help="If set, evaluate the checkpoint directly.",
    )
