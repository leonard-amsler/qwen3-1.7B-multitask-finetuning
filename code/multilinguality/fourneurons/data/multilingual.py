...
def get_xcopa() -> DatasetDict | Dataset | IterableDatasetDict | IterableDataset:
    ...
    cfg = load_config()
    return load_dataset(f"{cfg['hugging_face']['owner']}/xcopa", split="test")

def prepare_xcopa_jsonl(cfg: dict):
    """Prepares the XCOPA test dataset in the format expected by OpenCompass and saves locally."""
    xcopa_dataset = get_xcopa()
...
def format_ru_mmmlu(raw_dataset: DatasetDict) -> DatasetDict:
    ...
    # Rename "answer_letter" back to "answer"
    formatted_dataset = formatted_dataset.rename_column("answer_letter", "answer")
...
def format_mmmlu_prox(example):
    question = example["question"]
    options = [example[f"option_{i}"] for i in range(10) if example[f"option_{i}"] and example[f"option_{i}"] != "N/A"]
    answer = example["answer"]
    ...
    return {
        "prompt": prompt,
        "answer": answer,
        "idx": f"{example['lang']}_{example['category']}_{example['question_id']}",
        "subject": example["category"],
        "lang": example["lang"],
    }

def prepare_mmmlu_prox():
    languages = [
        "it",
        "es",
        "zh",
    ...
    columns = ['question_id', 'question', 'option_0', 'option_1', 'option_2', 'option_3', 'option_4', 'option_5', 'option_6', 'option_7', 'option_8', 'option_9', 'answer', 'answer_index', 'cot_content', 'category', 'src', 'question_id_src', 'lang']
    ...
    for lang in languages:
        lang_dataset = load_dataset("li-lab/MMLU-ProX", lang, split="test")
    ...
    dataset.push_to_hub(f"{cfg['hugging_face']['owner']}/mmmlu_prox", private=False)

    out_dir = Path(cfg["paths"]["data"]) / "multilingual" / "mmmlu_prox" / "splits"
    os.makedirs(out_dir, exist_ok=True)
    out_path = out_dir / "mmmlu_prox_test.jsonl"
...
def create_quicktest_split():
    """
    Creates a smaller quicktest split from the MMMLU test set for faster evaluation during development.
    """

    path = "/scratch/data/multilingual/mmmlu/splits/mmmlu_test.jsonl"
    out = "/scratch/data/multilingual/mmmlu/splits/mmmlu_quicktest.jsonl"

    with open(path) as f:
        rows = [json.loads(l) for l in f]

    np.random.seed(42)
    rows = np.random.choice(rows, size=min(1000, len(rows)), replace=False)

    with open(out, 'w') as f:
        for r in rows[:1000]:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")

    print(f"Created MMMLU quicktest split with {len(rows)} examples at {out}")

    pro_path = "/scratch/data/multilingual/mmmlu_prox/splits/mmmlu_prox_test.jsonl"
    pro_out = "/scratch/data/multilingual/mmmlu_prox/splits/mmmlu_prox_quicktest.jsonl"
    ...
    print(f"Created MMMLU-ProX quicktest split with {len(rows)} examples at {pro_out}")
...
if __name__ == "__main__":
    ...
    prepare_xcopa_jsonl(config)
    prepare_mmmlu(config)
    prepare_mmmlu_jsonl(config)
    prepare_mmmlu_prox()
    create_quicktest_split()